 ...
 * @brief 継承したメインループ
 */
void MainApp::Update(float df)							// de = 時間差
{
	if (GetKeyState('R') < 0) {
		// マスターデータ再読み込み
		Reload_();
	...
	// とまる～を操作
	objects_["Button"]->x = button_counter_;
	objects_["LabelOption"]->x = button_counter_;
...
{
	bezier_counter_.Set(100, 400, 60, cp);				// cp 動きの型
